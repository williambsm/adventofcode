...
function solution(input) {
    ...
    while (!winningBoard) {
        ...
        winningBoard = checkCombinations(boards, boardCombinations, bingoDrawn);
        bingoIndex++;
    }

    return calculateSolution(winningBoard, bingoDrawn);
}
...
function checkCombinations(boards, boardCombinations, bingoDrawn) {
    let winner = false;

    for (let i = 0; i < boardCombinations.length; i = i + 1) {
        boardCombinations[i].forEach((combination) => {
            if (combination.every(number => bingoDrawn.includes(number)) && !winner) {
                winner = boards[i];
            }
        });
    }
